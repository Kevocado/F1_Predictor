...
describe("index.css", () => {
  ...
  it("drops Inter, Titillium and the radial glow", () => {
    expect(css).not.toMatch(/Inter/);
    expect(css).not.toMatch(/Titillium/);
  ...
  it("declares the legacy aliases inline so they follow each sport's accent", () => {
    ...
    expect(css).toMatch(/@theme inline\s*\{[^}]*--color-f1-red:\s*var\(--color-pr-accent\)/);
  });
  it("maps every legacy F1 colour onto a family token", () => {
    const legacy = [...css.matchAll(/--color-(?:f1-[\w-]+|win|podium|dnf):\s*([^;]+);/g)].map((m) => m[1].trim());
